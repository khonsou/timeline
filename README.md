...
- **Agent API（v19 change-set 协议 + v2 追加）**：看板数据自动化的唯一写入口径——change-set 四端点（创建提案 / GET review / 原子 commit / cancel）+ item 级读写（items 过滤查询 / 单卡 GET / 白名单 PATCH，PATCH 与整板 PUT 支持可选 `If-Match` 版本保护）+ products / members 目录 + 逐字段审计（`audit_log` 含 actor / source / change_set_id / request_id + GET `/audit`）；每板每 IP 120 次/分钟限速；鉴权与人同一套密码换 token。v2 追加：M1 卡片表现字段 `bg_color` / `dimmed`、M2 统一分组（doc `groups[]` + 卡片 `group_id` + change-set 分组三 op `group_create` / `group_patch` / `group_delete`，上限 61）、M3 卡片前后关系（`pre_ids` 唯一写入源 / `post_ids` 只读镜像，整板 PUT 与建板在存储前做关系规范化）。完整说明见 [docs/agent-api.md](docs/agent-api.md)，可运行参考实现见 [examples/agent-quickstart.mjs](examples/agent-quickstart.mjs)。
